...
import { loadSettings, saveSettings, settingDefaults } from './settings.js';
import {
  TERMS_VERSION, WAIVER_TITLE, WAIVER_PREAMBLE, WAIVER_CLAUSES, INCOMPLETE_CLAUSES,
  WAIVER_VALIDITY_NOTE, HEARD_ABOUT_OPTIONS, WAIVER_VALID_MONTHS, ageOn,
} from './waiver-terms.js';
import {
...
import {
  ...
  addNoteImage, readNoteImage, deleteNoteImage,
  slotsFor, saveSlots, resetSlots, reportsFor, addWaiver, listWaivers,
} from './store.js';
 ...
 *
 * Content-Type is whatever the caller typed; it is not evidence. Sniffing the magic number
 * is what stops something being stored as a .png and served back to a browser as markup.
...
/** A positive integer from a query string, or null. Keeps junk out of the database. */
function posInt(v) {
  const n = Number(v);
...
export function createApiApp() {
  ...
  // Config a manager has changed is applied before any handler reads ARENA. Cached inside
  // loadSettings, so this is not a database round trip per request.
  app.use(async (_req, _res, next) => {
    try { await loadSettings(); } catch (e) { console.error('settings:', e.message); }
    next();
  });
  ...
  async function bookParty(req, res, { staff = false } = {}) {
    ...
    const saved = await addParty({
      ...
      // Only the desk may declare a deposit already taken.
      depositState: staff && body.depositState === 'paid' ? 'paid' : 'due',
      name: String(name).trim(),
      phone: String(phone).trim(),
      email: String(email || '').trim(),
  ...
  /* ----- Party deposit, via Stripe Checkout -----
   *
   * Same shape as the laser tag flow, and for the same reason: the party is created on the
   * way BACK, once Stripe confirms the deposit arrived. An abandoned payment therefore
   * leaves the room free rather than holding it for someone who never paid.
   *
   * The whole party rides in the session metadata, so the booking is rebuilt from STRIPE's
   * copy of it. Nothing the returning page claims is trusted except the session id.
   */
  app.post('/api/party-checkout-session', async (req, res) => {
    if (!stripeStatus().enabled) {
      return res.status(503).json({ error: 'Card payments are not configured on this server.' });
    }
    const b = req.body || {};
    ...
    if (!DATE_RE.test(str(b.dateISO))) return res.status(400).json({ error: 'Pick a date for the party.' });
    if (hoursFor(b.dateISO).closed) return res.status(400).json({ error: "We're closed that day. Please pick another date." });
    if (!str(b.name) || !str(b.phone)) return res.status(400).json({ error: 'We need a name and phone number to confirm your party.' });
    if (!validAge(b.age)) return res.status(400).json({ error: "Please tell us the birthday guest's age." });

    const priced = priceEventPackage(b.packageId, b.guests, { addOns: b.addOns, food: b.food });
    if (!priced.ok) return res.status(400).json({ error: priced.error });

    // Don't take a deposit for a slot that cannot seat them. Checked again on the way back,
    // because someone else can book while this customer is on Stripe's page.
    const board = (await partySlotsFor(b.dateISO, b.age, priced.guests))
      .find((s) => s.start === Number(b.slotStart));
    if (!board) return res.status(400).json({ error: "That party time isn't one we run on this day." });
    if (!board.available) return res.status(409).json({ error: board.reason });

    const base = safeOrigin(b.origin, req);
    ...
    try {
      const session = await getStripe().checkout.sessions.create({
        ...
        line_items: [{
          ...
          price_data: {
            currency: 'cad',
            unit_amount: ARENA.partyDepositCents,
            product_data: {
              name: `Party deposit — ${priced.name}`,
              description: `${longDateLabel(b.dateISO)}, ${fmtTime(Number(b.slotStart))} · ${priced.guests} guests · non-refundable`,
            },
          },
        }],
        customer_email: str(b.email) || undefined,
        success_url: `${base}/packages?deposit=success&session_id={CHECKOUT_SESSION_ID}`,
        cancel_url: `${base}/packages?deposit=cancelled`,
        metadata: {
          kind: 'party',
          dateISO: str(b.dateISO),
          slotStart: String(b.slotStart),
          packageId: priced.packageId,
          guests: String(priced.guests),
          age: String(b.age),
          name: str(b.name).slice(0, 200),
          phone: str(b.phone).slice(0, 60),
          email: str(b.email).slice(0, 200),
          // Stripe caps a metadata value at 500 characters; both of these are far shorter.
          addOns: JSON.stringify(priced.addOns.map((a) => a.id)),
          food: JSON.stringify((b.food || []).filter((f) => f && f.qty > 0).slice(0, 12)),
        },
      });
      res.json({ url: session.url });
    } catch (err) {
      console.error('party-checkout-session:', err.message);
      res.status(502).json({ error: "We couldn't start the deposit payment. Please try again, or call us to book." });
    }
  });

  app.post('/api/confirm-party-checkout', async (req, res) => {
    if (!stripeStatus().enabled) {
      return res.status(503).json({ error: 'Card payments are not configured on this server.' });
    }
    const sessionId = String((req.body || {}).sessionId || '');
    ...
    // Refreshing the confirmation page must not book — or charge for — a second party.
    const already = await findByCheckoutSession(sessionId);
    if (already) {
      const price = priceEventPackage(already.packageId, already.players, { addOns: already.addOns });
      return res.json({ booking: already, price, alreadyBooked: true });
    }
    ...
    try {
      ...
      console.error('confirm-party-checkout:', err.message);
      return res.status(502).json({ error: "We couldn't check that payment. If you were charged, call us and we'll sort it out." });
    }
    if (session.payment_status !== 'paid') {
      return res.status(402).json({ error: "That payment didn't complete, so nothing was booked." });
    ...
    let addOns = [];
    let food = [];
    try { addOns = JSON.parse(md.addOns || '[]'); } catch (_) { /* metadata is ours, but never trust a parse */ }
    try { food = JSON.parse(md.food || '[]'); } catch (_) { /* same */ }

    const priced = priceEventPackage(md.packageId, md.guests, { addOns, food });
    if (!priced.ok) return res.status(400).json({ error: priced.error });

    const saved = await addParty({
      dateISO: md.dateISO,
      slotStart: Number(md.slotStart),
      age: Number(md.age),
      guests: priced.guests,
      name: md.name || '',
      phone: md.phone || '',
    ...
    if (!saved.ok) {
      // Paid, but the room went while they were on Stripe's page. Say so plainly rather
      // than pretending it worked — a production build would auto-refund here.
      console.error(`⚠ DEPOSIT PAID BUT UNBOOKED — session ${session.id}: ${saved.error}`);
      return res.status(409).json({
        error: `Your deposit went through, but that party slot was taken while you were paying. Please call us on ${ARENA.phone} and we'll rebook you or refund you right away.`,
        paid: true,
      });
    }

    console.log(`🎉 Party ${saved.booking.code} — deposit paid via Stripe (${session.id})`);
    sendPartyConfirmation(saved.booking).catch((e) => console.error('party confirmation:', e.message));
    res.json({ booking: saved.booking, price: priced });
  });
  ...
  /** The terms, so the page renders exactly what gets stored against a signature. */
  app.get('/api/waiver-terms', (_req, res) => {
    res.json({
      version: TERMS_VERSION,
      title: WAIVER_TITLE,
      preamble: WAIVER_PREAMBLE,
      clauses: WAIVER_CLAUSES,
      incomplete: INCOMPLETE_CLAUSES,
      validityNote: WAIVER_VALIDITY_NOTE,
      heardAbout: HEARD_ABOUT_OPTIONS,
      validMonths: WAIVER_VALID_MONTHS,
      minimumAge: ARENA.recommendedAge,
    });
  });
  ...
  app.post('/api/waiver', async (req, res) => {
    ...
    const first = str(b.firstName), last = str(b.lastName), email = str(b.email);
    if (!first || !last) return res.status(400).json({ error: "Please enter the player's first and last name." });
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) return res.status(400).json({ error: 'Please enter a valid email address.' });
    if (!DATE_RE.test(str(b.dateOfBirth))) return res.status(400).json({ error: "Please enter the player's date of birth." });
    if (b.agreed !== true) return res.status(400).json({ error: 'Please agree to the terms and conditions to continue.' });

    const today = todayISO();
    const age = ageOn(str(b.dateOfBirth), today);
    if (age == null || age < 0) return res.status(400).json({ error: "That date of birth isn't valid." });
    if (age > 120) return res.status(400).json({ error: 'Please check the date of birth.' });

    // Clause 8: laser tag is 6 or older on the day of play.
    if (age < ARENA.recommendedAge) {
      return res.status(400).json({
        error: `Players must be ${ARENA.recommendedAge} years or older on the day of play.`,
      });
    }
    // Clause 1: under 18 needs a parent or guardian to sign along with them.
    if (age < 18 && (!str(b.guardianFirst) || !str(b.guardianLast))) {
      return res.status(400).json({
        error: 'Players under 18 need a parent or guardian to sign with them — please enter their name.',
      });
    }

    if (!str(b.address1) || !str(b.city) || !str(b.province) || !str(b.postalCode)) {
      return res.status(400).json({ error: 'Please complete the address.' });
    }
    if (!Array.isArray(b.heardAbout) || !b.heardAbout.length) {
      return res.status(400).json({ error: 'Please tell us how you heard about us.' });
    }

    const saved = await addWaiver({
      firstName: first, lastName: last, email,
      dateOfBirth: str(b.dateOfBirth),
      guardianFirst: str(b.guardianFirst), guardianLast: str(b.guardianLast),
      country: str(b.country) || 'Canada',
      address1: str(b.address1), address2: str(b.address2),
      city: str(b.city), province: str(b.province), postalCode: str(b.postalCode),
      heardAbout: b.heardAbout.filter((h) => HEARD_ABOUT_OPTIONS.includes(h)),
      comments: str(b.comments).slice(0, 2000),
      termsVersion: TERMS_VERSION,
      // Kept for a dispute: a waiver nobody can place is hard to rely on.
      ip: (req.headers['x-forwarded-for'] || '').split(',')[0].trim() || req.ip || '',
      userAgent: String(req.headers['user-agent'] || '').slice(0, 400),
    });
    if (!saved.ok) return res.status(saved.status || 400).json({ error: saved.error });

    const expires = parseISO(today);
    expires.setMonth(expires.getMonth() + WAIVER_VALID_MONTHS);

    console.log(`\u2705 Waiver ${saved.waiver.id} — ${first} ${last}`);
    res.json({ waiver: saved.waiver, expiresOn: isoDate(expires) });
  });

  app.get('/api/staff/waivers', async (req, res) => {
    const q = req.query || {};
    res.json(await listWaivers({
      search: String(q.search || ''),
      limit: Math.min(200, Math.max(1, Math.round(Number(q.limit) || 50))),
  ...
  app.get('/api/staff/settings', async (_req, res) => {
    const saved = await loadSettings({ force: true });
    res.json({
      saved,
  ...
  app.put('/api/staff/settings', async (req, res) => {
    ...
    if (!patch || typeof patch !== 'object') return res.status(400).json({ error: 'Nothing to save.' });

    // Guard the handful that would break the arena rather than merely change it.
    const a = patch.arena || {};
    const num = (k) => (a[k] === undefined ? null : Number(a[k]));
    if (num('capacity') !== null && (num('capacity') < 1 || num('capacity') > 200)) {
      return res.status(400).json({ error: 'Players per game must be between 1 and 200.' });
    }
    if (num('minPlayers') !== null && num('minPlayers') < 1) {
      return res.status(400).json({ error: 'A game needs at least 1 player to run.' });
    }
    if (num('capacity') !== null && num('minPlayers') !== null && num('minPlayers') > num('capacity')) {
      return res.status(400).json({ error: 'The minimum to run a game cannot exceed the arena capacity.' });
    }
    if (num('sessionStepMin') !== null && (num('sessionStepMin') < 5 || num('sessionStepMin') > 120)) {
      return res.status(400).json({ error: 'Games must launch between 5 and 120 minutes apart.' });
    }
    if (num('groupDiscountPct') !== null && (num('groupDiscountPct') < 0 || num('groupDiscountPct') > 100)) {
      return res.status(400).json({ error: 'The group discount must be between 0 and 100 percent.' });
    }
    for (const [day, pair] of Object.entries(patch.weeklyHours || {})) {
      if (!Array.isArray(pair) || pair.length !== 2) continue;
      const [o, c] = pair.map(Number);
      if (!(c > o)) {
        const names = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
        return res.status(400).json({ error: `${names[day]} closes before it opens.` });
      }
    }

    const r = await saveSettings(patch, String((req.body || {}).who || ''));
    if (!r.ok) return res.status(r.status || 400).json({ error: r.error });
    res.json({ ok: true, current: { arena: ARENA, packages: PACKAGES, eventPackages: EVENT_PACKAGES,
                                    weeklyHours: WEEKLY_HOURS, specialHours: SPECIAL_HOURS } });
  });
  ...
  /* ---------------- Note images ----------------
   *
   * Uploaded as a raw body rather than multipart, because multipart would mean a parser
   * dependency to move bytes we already have in one piece. express.json() above ignores
   * image content types, so the body arrives here untouched.
   *
   * 6mb is a backstop, not the working limit — the browser shrinks screenshots before they
   * get here. Vercel refuses request bodies over about 4.5mb at the edge, so anything
   * approaching this cap has already failed further out.
   */
  app.post('/api/staff/notes/:id/image',
    express.raw({ type: () => true, limit: '6mb' }),
    async (req, res) => {
      const buf = Buffer.isBuffer(req.body) ? req.body : null;
      if (!buf || !buf.length) return res.status(400).json({ error: 'No image was received.' });

      const mime = sniffImage(buf);
      if (!mime) {
        return res.status(415).json({ error: "That doesn't look like an image. PNG, JPEG, WebP and GIF work." });
      }

      const r = await addNoteImage(req.params.id, buf, {
        mime,
        width: posInt(req.query.w),
        height: posInt(req.query.h),
      });
      if (!r.ok) return res.status(r.status || 400).json({ error: r.error });
      res.json({ image: r.image });
    });

  app.get('/api/staff/notes/image/:id', async (req, res) => {
    const img = await readNoteImage(req.params.id);
    if (!img) return res.status(404).json({ error: 'That image is no longer here.' });
    res.set('Content-Type', img.mime);
    // An image is never edited in place — a new one gets a new id — so it can be cached
    // hard. `private` keeps it out of shared caches, since the portal has no sign-in yet.
    res.set('Cache-Control', 'private, max-age=31536000, immutable');
    res.send(img.bytes);
  });

  app.delete('/api/staff/notes/image/:id', async (req, res) => {
    const r = await deleteNoteImage(req.params.id);
    if (!r.ok) return res.status(r.status || 400).json({ error: r.error });
    res.json({ ok: true });
  });
