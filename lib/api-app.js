...
export function createApiApp() {
   ...
   */
  function publicPartySlots(date, age, guests) {
    return partySlotsFor(date, age, guests).map((s) => ({
      ...s,
  ...
  app.get('/api/party-slots', (req, res) => {
    ...
    res.json({
      ...
      slots: publicPartySlots(date, age, guests),
    });
  });
  ...
  app.post('/api/staff/send-reminders', async (req, res) => {
    ...
    const targets = bookingsFor(dateISO).filter((b) => b.kind !== 'block' && b.kind !== 'party');
