...
export function listNotes() {
  return [...notes]
    .sort((a, b) => (a.done === b.done ? (a.createdAt < b.createdAt ? 1 : -1) : (a.done ? 1 : -1)))
    .map((n) => ({ ...n, images: [] }));
}
 ...
 * Note images
 *
 * Not supported here. Screenshots belong in Supabase Storage, and holding megabytes of
 * image in a process that forgets everything on restart would be a worse answer than a
 * plain no. Says so rather than failing obscurely.
 * ------------------------------------------------------------------ */

const NO_STORAGE = 'Image attachments need the database. This server is running on the in-memory store.';

export function addNoteImage() { return { ok: false, status: 503, error: NO_STORAGE }; }
export function readNoteImage() { return null; }
export function deleteNoteImage() { return { ok: false, status: 503, error: NO_STORAGE }; }
