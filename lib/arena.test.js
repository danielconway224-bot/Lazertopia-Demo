...
import {
  ...
  priceEventPackage, perPersonFor, fmtTime, money, isoDate, parseISO, isPastSession,
...
import {
  ...
  partySlotsFor, addParty, releaseHeldGame, holdGame, isHeld, bookingsFor,
} from './demo-store.js';

import { stripeStatus } from './payments.js';
import { safeOrigin, sniffImage } from './api-app.js';
import {
...
test('a run can be 1 to 10 games, and no more', () => {
  for (let g = 1; g <= ARENA.maxGames; g++) {
    assert.equal(priceBooking({ players: 4, games: g }).ok, true, `${g} games should be bookable`);
  }
  assert.equal(priceBooking({ players: 4, games: 0 }).ok, false);
  assert.equal(priceBooking({ players: 4, games: 11 }).ok, false);
  assert.match(priceBooking({ players: 4, games: 11 }).error, /between 1 and 10/);
});

test('the published tiers hold, and games past them are the single-game price', () => {
  // The deal improves to three games and then stops, rather than compounding away.
  assert.equal(perPersonFor(1), 849);
  assert.equal(perPersonFor(2), 1549);
  assert.equal(perPersonFor(3), 2149);
  assert.equal(perPersonFor(4), 2149 + 849, 'a fourth game adds the single-game price');
  assert.equal(perPersonFor(10), 2149 + 7 * ARENA.extraGameCents);
  assert.equal(money(perPersonFor(10)), '$80.92');

  // Every extra game costs the same, so the curve is straight past the third.
  for (let g = 5; g <= ARENA.maxGames; g++) {
    assert.equal(perPersonFor(g) - perPersonFor(g - 1), ARENA.extraGameCents, `step to ${g}`);
  }
});

test('a long run is priced per player', () => {
  const p = priceBooking({ players: 4, games: 10 });
  assert.equal(p.perPersonCents, 8092);
  assert.equal(p.totalCents, 4 * 8092);
  assert.equal(money(p.totalCents), '$323.68');
});
...
test('an untouched date has no bookings at all', () => {
  // The store used to invent a plausible day's trade on first access. It must not: a
  // booking nobody made is a customer turned away from a seat that was always free.
  const rows = availabilityFor('2026-10-01');
  assert.ok(rows.length > 0, 'the day still has game sessions');
  assert.deepEqual(rows.map((s) => s.taken), rows.map(() => 0), 'every session starts empty');
  assert.deepEqual(bookingsFor('2026-10-01'), []);
  assert.deepEqual(partySlotsFor('2026-10-01').flatMap((s) => s.parties), []);
});

test('stats add up for the staff dashboard', () => {
  const D5 = '2026-09-19';                       // its own date, so ordering cannot matter
  assert.equal(statsFor(D5).playersExpected, 0, 'nothing booked yet');

  const slot = availabilityFor(D5).find((s) => !s.held && !s.past && !s.cutoff);
  assert.equal(addBooking({
    dateISO: D5, startMin: slot.startMin, games: 1, players: 6,
    name: 'Stats', totalCents: 5094,
  }).ok, true);

  const s = statsFor(D5);
  assert.equal(s.totalSessions, sessionsFor(D5).length);
  assert.equal(s.playersExpected, 6);
  assert.equal(s.revenueCents, 5094);
});
...
test('nothing but a party can land in a held time', () => {
  const d = '2026-11-27';                        // a Friday
  for (const m of heldGamesFor(d)) {
    const r = addBooking({ dateISO: d, startMin: m, games: 1, players: 4, name: 'Public' });
    assert.equal(r.ok, false, `${fmtTime(m)} should be closed to the public`);
    assert.match(r.error, /reserved for birthday parties/);
  }
  // And the sheet agrees: no non-party booking is sitting in one.
  for (const s of gameSheetFor(d)) {
    if (!heldGamesFor(d).includes(s.startMin)) continue;
    assert.deepEqual(s.bookings.filter((b) => b.kind !== 'party'), []);
  }
});
...
/* ---------------------------------------------------------------- *
 * Uploaded image sniffing
 *
 * The whole point is that Content-Type is not evidence. These check the bytes decide.
 * ---------------------------------------------------------------- */
...
test('sniffImage recognises the formats we store', () => {
...
test('sniffImage rejects anything that is not one of them', () => {
  // A script dressed as an image is the attack this exists to stop.
  assert.equal(sniffImage(Buffer.from('<script>alert(1)</script>')), null);
  assert.equal(sniffImage(Buffer.from('%PDF-1.7 trailing bytes here')), null);
  assert.equal(sniffImage(Buffer.alloc(64)), null);
  assert.equal(sniffImage(Buffer.from('RIFF____AVI ')), null);   // RIFF, but not WebP
});

test('sniffImage survives junk input instead of throwing', () => {
  assert.equal(sniffImage(null), null);
  assert.equal(sniffImage(Buffer.alloc(0)), null);
  assert.equal(sniffImage(Buffer.from([0x89, 0x50])), null);     // too short to judge
});
