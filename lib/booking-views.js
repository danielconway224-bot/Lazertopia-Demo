...
/**
 * The game times a booking actually occupies.
 *
 * Ordinary bookings run back-to-back, so N games from the start time. A PARTY does not: its
 * games are whatever its room slot plays, and those are not consecutive — Saturday's 1–3
 * slot plays 1:15 and 1:45, not 1:15 and 1:30. Spanning it like a normal booking put the
 * party in a public game it was never in and left a held game looking empty.
 *
 * Derived from the slot rather than stored, so it cannot drift from the schedule.
 */
export function spanOf(b, dateISO, slots = null) {
  if (b.kind === 'party' && b.slotStart != null) {
    const slot = (slots || partyRoomSlotsFor(dateISO)).find((x) => x.start === b.slotStart);
    const games = slot ? slot.games : gamesInSlot(dateISO, b.slotStart);
    if (games.length) return games;
  }
  return sessionSpan(dateISO, b.startMin, b.games);
}

/** Seats taken in one session, counting every booking whose span covers it. */
export function seatsAt(bookings, dateISO, startMin, slots = null) {
  let taken = 0;
  for (const b of bookings) {
    ...
    if (spanOf(b, dateISO, slots).includes(startMin)) taken += b.players;
  }
 ...
 */
export function availabilityFrom(bookings, dateISO, heldAt, slots = null) {
  return sessionsFor(dateISO).map((startMin) => {
    const taken = seatsAt(bookings, dateISO, startMin, slots);
...
export function gameSheetFrom(bookings, dateISO, heldAt, defaultHeld, slots = null) {
  return sessionsFor(dateISO).map((startMin) => {
    const span = bookings.filter((b) =>
      b.kind === 'block' ? b.startMin === startMin : spanOf(b, dateISO, slots).includes(startMin));
    const taken = seatsAt(bookings, dateISO, startMin, slots);
    return {
      startMin,
      taken,
 ...
 */
export function partySlotsFrom(bookings, dateISO, age = null, guests = null, slotList = null) {
  const parties = bookings.filter((b) => b.kind === 'party' && b.slotStart != null);
  const slots = slotList || partyRoomSlotsFor(dateISO);
 ...
 * Reports
 *
 * Two different questions that look the same and are not:
 *
 *   "How much did we SELL last week"  — grouped by the day the booking was PLACED.
 *   "How busy were we last week"      — grouped by the day the games RAN.
 *
 * A party booked in June for August belongs to June's sales and August's trade. Reporting
 * both off one date is the classic way to make a quiet month look busy.
 * ------------------------------------------------------------------ */

/** A booking counts as paid when Stripe confirmed money for it. */
const isPaid = (b) => !!(b.paymentIntentId || b.checkoutSessionId);

/**
 * @param {object}   o
 * @param {Array}    o.bySession   bookings whose GAME DATE falls in the window
 * @param {Array}    o.bySale      bookings PLACED in the window
 * @param {string[]} o.dates       every date in the window, oldest first
 * @param {(iso:string)=>number[]} o.sessionsOn  game times that run on a date
 */
export function buildReports({ bySession, bySale, dates, sessionsOn }) {
  const live = (rows) => rows.filter((b) => b.kind !== 'block');
  const sessionRows = live(bySession);
  const saleRows = live(bySale);

  const blank = () => Object.fromEntries(dates.map((d) => [d, 0]));

  /* ---- Sales: by the day the booking was placed ---- */
  const salesByDay = blank();
  let grossSold = 0;
  for (const b of saleRows) {
    const day = (b.createdAt || '').slice(0, 10);
    if (day in salesByDay) salesByDay[day] += b.totalCents;
    grossSold += b.totalCents;
  }
  const paidSales = saleRows.filter(isPaid);
  const paidCents = paidSales.reduce((t, b) => t + b.totalCents, 0);

  /* ---- Trade: by the day the games run ---- */
  const tradeByDay = blank();
  const guestsByDay = blank();
  const sizes = {};
  let guests = 0;
  let seatsSold = 0;
  let seatsOffered = 0;
  const sessionsWithBookings = new Set();

  for (const b of sessionRows) {
    tradeByDay[b.dateISO] = (tradeByDay[b.dateISO] || 0) + b.totalCents;
    guestsByDay[b.dateISO] = (guestsByDay[b.dateISO] || 0) + b.players;
    guests += b.players;
    sizes[b.players] = (sizes[b.players] || 0) + 1;
    // Seats sold counts every game a booking occupies, not just the one it starts in.
    for (const m of spanOf(b, b.dateISO)) {
      seatsSold += b.players;
      sessionsWithBookings.add(`${b.dateISO}@${m}`);
    }
  }
  for (const d of dates) seatsOffered += sessionsOn(d).length * ARENA.capacity;

  const totalSessions = dates.reduce((t, d) => t + sessionsOn(d).length, 0);

  /* ---- Parties ---- */
  const parties = sessionRows.filter((b) => b.kind === 'party');
  const byPackage = {};
  for (const p of parties) {
    const k = p.packageName || 'Unnamed package';
    if (!byPackage[k]) byPackage[k] = { name: k, count: 0, guests: 0, cents: 0 };
    byPackage[k].count += 1;
    byPackage[k].guests += p.players;
    byPackage[k].cents += p.totalCents;
  }

  /* ---- Where the money came from ---- */
  const bySource = { online: 0, walkin: 0, party: 0 };
  for (const b of sessionRows) if (bySource[b.kind] != null) bySource[b.kind] += b.totalCents;

  const owed = sessionRows.filter((b) => !isPaid(b)).reduce((t, b) => t + b.totalCents, 0);

  return {
    sales: {
      transactions: saleRows.length,
      grossCents: grossSold,
      paidCents,
      // Counted off the SALE rows, not the session rows. Mixing the two put a paid total
      // next to a count of nought, because the booking was sold this month and plays next.
      paidCount: paidSales.length,
      unpaidCount: saleRows.length - paidSales.length,
      unpaidCents: grossSold - paidCents,
      byDay: dates.map((d) => ({ date: d, cents: salesByDay[d] })),
    },
    trade: {
      bookings: sessionRows.length,
      sessionsBooked: sessionsWithBookings.size,
      sessionsOffered: totalSessions,
      grossCents: sessionRows.reduce((t, b) => t + b.totalCents, 0),
      byDay: dates.map((d) => ({ date: d, cents: tradeByDay[d], guests: guestsByDay[d] })),
      bySource,
    },
    payments: {
      // "Collected" means a card actually cleared. Everything else is owed at the desk,
      // which is money the venue has not got yet however firm the booking looks.
      collectedCents: sessionRows.filter(isPaid).reduce((t, b) => t + b.totalCents, 0),
      dueAtDeskCents: owed,
      onlineCount: sessionRows.filter(isPaid).length,
      deskCount: sessionRows.filter((b) => !isPaid(b)).length,
    },
    guests: {
      total: guests,
      avgPartySize: sessionRows.length ? Number((guests / sessionRows.length).toFixed(1)) : 0,
      sizes: Object.entries(sizes)
        .map(([n, count]) => ({ players: Number(n), count }))
        .sort((a, b) => a.players - b.players),
    },
    fill: {
      seatsSold,
      seatsOffered,
      pct: seatsOffered ? Math.round((seatsSold / seatsOffered) * 100) : 0,
      sessionsBooked: sessionsWithBookings.size,
      sessionsOffered: totalSessions,
    },
    parties: {
      count: parties.length,
      guests: parties.reduce((t, p) => t + p.players, 0),
      grossCents: parties.reduce((t, p) => t + p.totalCents, 0),
      depositsDueCents: parties.filter((p) => p.depositState !== 'paid')
        .reduce((t, p) => t + (p.depositCents || 0), 0),
      byPackage: Object.values(byPackage).sort((a, b) => b.cents - a.cents),
    },
  };
}
