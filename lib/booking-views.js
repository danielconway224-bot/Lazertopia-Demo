 ...
 */
export function spanOf(b, dateISO) {
  if (b.kind === 'party' && b.slotStart != null) {
    const games = gamesInSlot(dateISO, b.slotStart);
...
export function seatsAt(bookings, dateISO, startMin) {
  ...
  for (const b of bookings) {
    ...
    if (spanOf(b, dateISO).includes(startMin)) taken += b.players;
  }
...
export function gameSheetFrom(bookings, dateISO, heldAt, defaultHeld) {
  return sessionsFor(dateISO).map((startMin) => {
    const span = bookings.filter((b) =>
      b.kind === 'block' ? b.startMin === startMin : spanOf(b, dateISO).includes(startMin));
    const taken = seatsAt(bookings, dateISO, startMin);
    return {
      startMin,
      taken,
