...
//
//  1. TEST KEYS ONLY, until launch. A live key would charge real cards while the system
//     is still being built, so it is refused outright rather than warned about. Relax
//     this deliberately when going live — see the go-live checklist in the README.
...
export function stripeStatus(env = process.env) {
  ...
  if (LIVE.test(secret) || LIVE.test(publishable)) {
    return {
      enabled: false,
      publishableKey: null,
      problem: 'LIVE Stripe keys detected. This system is not launched yet — use test keys (sk_test_ / rk_test_ / pk_test_) only.',
    };
  }
  if (!secret || !publishable) {
    return {
