...
export const ARENA = {
  ...
  // Games beyond the published three are each this much more per player. The tiers are the
  // deal — 2 and 3 games are cheaper per game than buying them singly — and past that the
  // discount stops rather than compounding away to nothing.
  extraGameCents: 849,
  maxGames: 10,
 ...
 */
/**
 * What one player pays for N games.
 *
 * The three published tiers are a deal that improves with each game. Past the third the
 * deal stops rather than compounding: every further game is the plain single-game price.
 * Left as an explicit function rather than folded into a table, because the shape of the
 * ladder is a commercial decision and it should be readable.
 */
export function perPersonFor(games) {
  const g = Math.round(Number(games) || 0);
  const tier = PACKAGES.find((p) => p.games === g);
  if (tier) return tier.cents;
  const top = PACKAGES[PACKAGES.length - 1];
  if (g <= top.games) return 0;
  return top.cents + (g - top.games) * ARENA.extraGameCents;
}
...
export function priceBooking({ players, games }) {
  ...
  if (!isFinite(g) || g < 1 || g > ARENA.maxGames) {
    return { ok: false, error: `Choose between 1 and ${ARENA.maxGames} games.` };
  }

  const perPersonCents = perPersonFor(g);
  const subtotalCents = perPersonCents * n;
